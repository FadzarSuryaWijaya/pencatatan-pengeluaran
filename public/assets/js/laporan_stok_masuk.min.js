...
$(function() {
    ...
    function cb(start, end, label) { // Added 'label' parameter
        if (label === 'Semua Waktu') {
            $('#reportrangesm span').html('Semua Waktu');
            // Jika 'Semua Waktu' dipilih, kirim parameter kosong
            laporan_stok_masuk.ajax.url(filterUrl + '?start_date=&end_date=').load();
        } else {
            $('#reportrangesm span').html(start.format('MMMM D, YYYY') + ' - ' + end.format('MMMM D, YYYY'));
            // Load data with filter when dates change
            laporan_stok_masuk.ajax.url(filterUrl + '?start_date=' + start.format('YYYY-MM-DD') + '&end_date=' + end.format('YYYY-MM-DD')).load();
        }
    }

    $('#reportrangesm').daterangepicker({
        startDate: start,
        endDate: end,
        ranges: {
            'Today': [moment(), moment()],
